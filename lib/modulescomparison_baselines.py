...
class ModevalKnownmodules:
    ...
    def save(self, name, full=True):
        parent_dir = "../results/modeval_knownmodules/"
        output_file = parent_dir + name + ".tsv"

        Path(output_file).parent.mkdir(parents=True, exist_ok=True)
        self.scores.to_csv(output_file, sep="\t")

        if full:
            output_file = parent_dir + name + ".json"
            json.dump(self.scores_full, open(output_file, "w"), cls=JSONExtendedEncoder)
...
def modevalworker(setting, scores):
    ...
    for regnet_name in dataset["knownmodules"].keys():
        for knownmodules_name in dataset["knownmodules"][regnet_name].keys():
            ...
            settingscores_goldstandard = modevalscorer(modules, knownmodules, None) # baselinesoi)
...
class ModevalCoverage:
    ...
    def save(self, name, full=True):
        parent_dir = "../results/modeval_coverage/"
        output_file = parent_dir + name + ".tsv"
